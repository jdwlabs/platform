...
## Finding 7 — there is no such thing as landing the chart unsynced

The sizing record's step 1 is "land the chart, do not sync it", with its blast
radius given as "none. Nothing is applied." Its step 2 then verifies the
rendered manifest before step 3 performs the sync. The ordering is the whole
safety argument: look at what ArgoCD is about to run, then let it run.

That ordering is not available in this repo. Services are generated by the
tenant envelope's ApplicationSet, which sets:
...
Adding a service entry to `tenants/platform/tenant.yaml` therefore generates
an Application that syncs itself. The merge *is* the sync. There is no window
between them in which to inspect a rendered manifest, and step 1's "nothing is
applied" describes a repo that does not sync automatically.

The verification step 2 asks for still has value, but it has to happen
*before* the merge rather than after: render the chart locally against the
values file and confirm `cni.exclusive`, `resources` and `policyAuditMode`
there. That is a weaker check than reading what ArgoCD itself produced, and
the gap is worth naming rather than papering over — the local render uses the
same chart and values but not the same renderer.

The practical consequence is that the merge of the chart-landing change is the
highest-consequence moment in the whole rollout, not the inert one the sizing
record describes. Everything after it is gated behind audit mode; that merge
is gated behind nothing.

## Decision

**1. The rollout is five steps, not eleven.** The sizing record's steps 1 and
2 collapse into one — render and verify locally, then merge, because the merge
syncs. Its steps 3 through 5 — check scheduling before health, restart every
pod and assert the count, observe for a minimum of seven days — are unchanged
and remain the substance of the work. Its steps 6 through 11 are replaced by:
