...
| `BackupArtifactEmpty` (critical) | The newest artifact is zero bytes: the job completed successfully and wrote nothing. This is the JDWLABS-338 shape and it is never a false positive — a producer publishes `0` deliberately when its store is empty, including before its first run. Read the last job's logs for a failed archive step, and confirm the source it was reading is where it is supposed to be. |
