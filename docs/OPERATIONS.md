...
| `JdwillmsenMinecraftPresenceStale` (warning) | The presence leader has not completed a tick that read both its overrides and the actor statuses for over five minutes, and has held that for five more. Parks, expiries and wakes are stuck, and desired state and override ages hold their last values. Check the leader agent's logs: `presence_tick_skipped` is a failed override read, `presence_status_read_failed` a failed status read, and `presence_store_unready` means its presence tables are not migrated. Then check the Postgres cluster behind it. An unreadable status table reads every bot as out of the world, so `JdwillmsenMinecraftAfkBotsDegraded` may page for bots that are fine once the outage passes thirty minutes. A new leader seeds its stamp at 0, so one that never reads the store warns too; one that reads it within the hold does not. With no stamp at all (presence switched off, no leader yet, or the agent gone) it is silent, and the agent connection rules cover the last. |
