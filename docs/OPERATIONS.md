...
| A tenant's `<tenant>-dashboards` folder is readable by another tenant's team | The folder Git Sync creates has no folder-RBAC of its own, so it inherits Grafana's default Viewer/Editor grants — a synced tenant folder is instance-wide readable until something replaces its permission list. tenant-envelope's observability Job does that, keyed off `observability.grafana.gitSyncFolder` in the tenant's `tenant.yaml`; an unset key means the Job never touches the folder and never says so. Check the key first, then the Job's log in `monitoring` for `folder <uid> permissions set`. The Job waits up to 5 minutes for the folder to appear and fails rather than skipping, because "the folder is not there yet" and "the folder is there and open" are the same observation without asking. |
