...
| A tenant folder went world-readable after someone deleted its repository | Expected consequence, and the reason `gitsync delete` refuses it. With `sync.target: folder` the folder is a resource the repository owns, so the remove-orphan-resources finalizer takes it, and the one Grafana recreates carries the inherited Admin/Editor/Viewer grants until that tenant's PostSync hook runs again — which nothing schedules on its own, because a hook Job's absence is not drift and `selfHeal` never fires. Re-sync `governance-<tenant>` (`kubectl -n argocd annotate app governance-<tenant> argocd.argoproj.io/refresh=hard --overwrite`) or run `platformctl gitsync recreate --repository <name> --confirm`, which re-syncs it for you and says so. |
