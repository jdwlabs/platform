...
| 4 | `vault-seed`     | Write all platform + tenant kv paths                              | Provide secret values via prompts or `PLATFORMCTL_*` env. One field alone: `bootstrap seed <spec> --field <name> --from-file <path>`, which needs no terminal ([OPERATIONS.md §6.1](OPERATIONS.md#61-seeding-one-credential-with-no-tty)) |
