...
|---|---|---|---|---|---|
| ai-sre/platform-litellm | 512Mi req, 1988Mi used, 1 replica | 2Gi req = 2Gi limit, cpu 100m req, no cpu limit → **Burstable** overall (memory-only Guaranteed) | **keep-as-is** | n/a | request: trustworthy (platform#214); CPU utilization pattern: unverified |
| ai-sre/platform-holmes-holmes | 2048Mi req, 853Mi used, 1 replica | 1536Mi req = 1536Mi limit, cpu 100m req, no cpu limit → **Burstable** overall (memory-only Guaranteed) | **keep-as-is** | n/a | request: trustworthy (platform#214, itself confirming the Phase 1 hit-list trim); CPU utilization pattern: unverified |
...
3. **A second replica costs real memory for no proven benefit.** litellm runs Guaranteed **on memory only** (`requests.memory == limits.memory == 2Gi`, deliberately, per the comment already in that file) — the CPU request has no matching limit, so the pod's overall QoS class is Burstable, not Guaranteed. Scaling out doubles the memory reservation cluster-wide — directly against this epic's own memory-conservation objective — while doing nothing for the actual constraint above.
...
- The one documented "burst" for this workload is a **memory** burst inside a single investigation (tool-heavy runs approaching ~1.5Gi), which Phase 2 already right-sized with `requests.memory == limits.memory` specifically so memory isn't the first thing evicted under node pressure — that in-file comment calls this "Guaranteed QoS", but the pod's CPU request has no matching limit, so the actual class is Burstable overall; the eviction-priority argument still holds (memory usage never exceeds its own request, which is what keeps it out of the eviction-eligible pool), it just isn't Guaranteed-class protection. That reasoning is already recorded in `tenants/platform/services/holmes/values.yaml` and doesn't change here.
...
Neither `values-non.yaml` nor `values-prd.yaml` overrides this, so both environments already run a live HorizontalPodAutoscaler (rendered by the shared `common` library chart's `_hpa.yaml`) scaling 1→3 replicas on 80% CPU or 80% memory utilization against the request. Phase 1's baseline captured both environments at 1 running replica while the request was still 1024Mi, where 224-331Mi usage sat at 22-32% of the trigger — genuinely nowhere close. That's no longer the current picture: Phase 2 (platform#213) halved the request to 512Mi, which halves the trigger's absolute value too (from ~819Mi to ~410Mi). Against that new denominator, prd's 331Mi baseline reading is **65% of request**, 15 points below the 80% trigger, not "never got close." This is computable from repo values alone, not something that needs a live re-baseline — worth tracking, since the JVM sizes its heap off the 2Gi *limit* rather than the 512Mi request, so RSS drift into that gap is plausible. Watch for HPA scale-out events; each one pays the ~5-minute startup warmup the startupProbe was added for (per the prior outage that prompted it), and memory-driven scale-out doesn't reduce per-pod JVM RSS, so a HPA that trips on memory can ratchet toward `maxReplicas: 3` without ever scaling back down.
