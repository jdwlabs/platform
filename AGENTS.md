...
- Deleting a tenant repository deletes its folder (the remove-orphan-resources finalizer owns it) and the folder that comes back is open until that tenant's hook re-runs. `recreate` re-syncs `governance-<tenant>` alongside `platform-grafana` and reports it; `gitsync delete` requests no sync, so it **refuses** a repository a tenant claims unless `--accept-open-folder` is passed
