...
- A synced folder is created without folder-RBAC, which means readable by every Grafana user. A tenant's `<tenant>-dashboards` folder is granted to its team by tenant-envelope via `observability.grafana.gitSyncFolder` in `tenant.yaml` — a new tenant repository without that key ships an open folder
- Only `platform-dashboards` is health-gated by the apply Job. It is an ArgoCD `Sync` hook, so a hard gate on a tenant repository would fail the whole `platform-grafana` sync; tenant repositories are warned about and left to `gitsync status`
